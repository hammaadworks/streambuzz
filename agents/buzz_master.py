...
to retrieve and store information about active streams and chat replies.

Attributes:
    model (str): The name of the language model to be used by the agent.
    name (str): The name of the agent, "buzz_master_agent".
    end_strategy (str): The strategy used to end the agent's execution, set to "early".
    model_settings (ModelSettings): Configuration settings for the language model, including temperature.
    ...
    result_tool_description (str): The description of the result tool.
    result_retries (int): The number of times to retry the result tool if it fails.
...
async def get_current_buzz(ctx: RunContext[str]) -> str:
    """
    Retrieves the current buzz for a given session ID.
...
async def store_reply(ctx: RunContext[str], reply: str) -> str:
    """
    Stores a user's reply for a given session ID.

    The reply is associated with the active stream and is marked as not yet written to the live chat.
    ...
    Returns:
        str: A confirmation message indicating that the reply has been acknowledged.

    Raises:
        UserError: If no active stream is found or if there is an error storing the reply.
    """
    try:
